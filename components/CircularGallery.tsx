...
function createTextTexture(
  ...
  if (!context) {
    return { texture: new Texture(gl), width: 0, height: 0 };
  }
  context.font = font;
...
class App {
  ...
  private createMedias(
    ...
    const defaultItems: GalleryItem[] = [
      { image: "https://picsum.photos/seed/1/800/600", text: "" },
      { image: "https://picsum.photos/seed/2/800/600", text: "" },
      { image: "https://picsum.photos/seed/3/800/600", text: "" }
    ];
    const galleryItems = items && items.length ? items : defaultItems;
    // Duplicate items to ensure there are enough for seamless looping when
    // the user supplies only a few photos.
    const minLoopItems = 6;
    let looped: GalleryItem[] = [...galleryItems];
    while (looped.length < minLoopItems) looped = looped.concat(galleryItems);
    this.mediasImages = looped.concat(looped);
